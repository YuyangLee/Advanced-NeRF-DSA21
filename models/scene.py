'''
LastEditTime: 2022-01-13 18:19:12
Description: Scene
...
class Scene:
    def __init__(self, range=[[-1., 1.]]*3, resolution_neglog=-5, oct_default_depth=3, mode="octree_lite", device='cuda'):
        self.mode = mode
    ...
    def reconstruct_volume(self, rec_kwargs):
        ...
        max_chunk_size = self.resolution * 2**5
        min_chunk_size = 2**(-rec_kwargs['resolution_neglog']) * 2**4
        threshold = 0.0001
        sparse_resolution = 2**(-4)
        
        time_start = time.time()
        if self.mode == "octree":
            sparse_volume = Volume(self.range, sparse_resolution).reconstruct(rec_kwargs['rec_network'], rec_kwargs['network_query_fn'])
            
            query_nodes = self.octree.subdivide(min_chunk_size, lambda node: subdivide_discriminator(node, sparse_volume, sparse_resolution, threshold))
            ...
            query_ranges = ol.subdivide(self.range, self.min_divide, min_chunk_size, lambda range: ol.subdivide_discriminator_lite(range, sparse_volume, sparse_resolution, threshold))
